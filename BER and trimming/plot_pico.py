...
#         second = x[i]
#         if not second > first:
#             print(second, first, x[i], y1[i], y2[i], i)
#             break
#         first = x[i]
...
end_index = None
for i in range(950_000, len(elist)):
...
# plt.plot(x, y1, label='Sent signal')
plt.plot(x, y2, label='Received signal')
# plt.plot(x, b1, label='Sent bit')
# plt.plot(x, b2, label='Received bit')
# plt.plot(x, e, label='XOR')
...
for i in range(start_index,len(bb1)):
    ...
    # print(f"sent: {bb1[i]} - received: {bb2[i]}")



# print(ee)
# print(len(counts))
# print(counts.count(47))
